...
void async() {
	...
	int value = 42;
	// Fast
	std::future<int> future1 = std::async(std::launch::async, doSomeJob, 41); // 2s
	// Fast
	std::future<int> future2 = std::async(std::launch::async, doSomeJob, 42); // 2s

	auto future3 = std::async(std::launch::deferred, doSomeJob, 43); // 2s 
	...
	//destructor call
	std::async(std::launch::async, doSomeJob, 88);
}
